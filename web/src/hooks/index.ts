...
export const useCrud = (options: IHooksOptions) => {
	...
	const deleteHandle = (key: number | string, field: string = 'id') => {
		if (!state.deleteUrl) {
			return
		}
		...
		ElMessageBox.confirm('确定进行删除操作?', '提示', {
			...
			.then(() => {
				service.delete(state.deleteUrl + `?${field}=` + key).then(() => {
	...
	const deleteBatchHandle = (key?: number | string) => {
		let data: { id_arr: any[] } = { id_arr: [] }
		if (key) {
			data.id_arr = [key]
		} else {
			data.id_arr = state.dataListSelections ? state.dataListSelections : []

			if (data.id_arr.length === 0) {
	...
	const downloadHandle = (url: string, filename?: string, method: string = 'POST'): Promise<any> => {
		return axios({
			responseType: 'blob',
			url: import.meta.env.VITE_API_URL + '/admin_api/down_file?action=downFile&filePath=' + url + '&access_token=' + cache.getToken(),
			method: method
		})
			.then((res: any): any => {
				// 创建a标签
