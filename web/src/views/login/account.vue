...
let isLimitOpenCaptcha = ref(false)
...
const onCaptcha = async () => {
	...
	if (!data.openCaptcha) {
		isLimitOpenCaptcha.value = true
	} else {
		isLimitOpenCaptcha.value = false
	}
	loginForm.key = data.captchaId
	captchaBase64.value = data.picPath
	if (isLimitOpenCaptcha.value) {
		ElMessage.error('请稍候请求验证码')
		return
	}
}
